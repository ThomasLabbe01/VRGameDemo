...
tcp_socket.bind(('0.0.0.0', server_port))  # Bind to any available network interface
tcp_socket.listen(1)  # Listen for one incoming connection

print("Waiting for the TCP client to connect...")
print(np.array2string(f().flatten().astype(int),formatter={'int': lambda x: f"{x:04d}"},separator=';')[1:-1])  # Removing the leading space and trailing comma)
